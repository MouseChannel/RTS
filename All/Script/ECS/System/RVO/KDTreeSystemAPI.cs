...
namespace RVO
{
    public partial class KDTreeSystem
    {
        ...
        private void GetRangeObstacleVertices(Agent agent, ObstacleTreeNode node, FixedInt rangeSq, NativeList<ObstacleNeighbor> obstacleNeighbors)
        {
            ...
            {
                if (node.left_index != -1) GetRangeObstacleVertices(agent, obstacleTree_[node.left_index], rangeSq, obstacleNeighbors);
            }
            else
            {
                if (node.right_index != -1) GetRangeObstacleVertices(agent, obstacleTree_[node.right_index], rangeSq, obstacleNeighbors);
            }
