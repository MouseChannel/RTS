...
public partial class KDTreeSystem
{
    ...
    [BurstCompile]
    public partial struct UpdateAgent : IJobEntity
    {
        [ReadOnly]
        ...
        {
            this.agent = a;
            // Debug.Log(string.Format("{0}", entityInQueryIndex));
            ...
            // ecbPara.SetComponent<Agent>(entityInQueryIndex, entity, agent);
            // agents[entityInQueryIndex] = agent;


            ecbPara.SetComponent<FOWUnit>(entityInQueryIndex, entity, new FOWUnit { position = agent.position_, range = 10 });
    ...
    public struct UpdateAgentJobParallel : IJobParallelFor
    {
        ...
        public void Execute(int index)
        {
            ...
            ecbPara.SetComponent<FOWUnit>(index, entity, new FOWUnit { position = agent.position_, range = 10 });
    ...
    public struct ComputeEnemyUnitJob : IJob
    {
        ...
        {
            int agentNo = -1;
            FixedInt distance = FixedCalculate.superBigValue;
    ...
    private static void ComputeObstacleNeighbor(NativeList<ObstacleVertice> obstacles, NativeList<ObstacleVerticeTreeNode> obstacleTree, ObstacleVerticeTreeNode node, Agent agent, ref FixedInt rangeSq, NativeList<ObstacleVerticeNeighbor> obstacleNeighbors)
    {
        if (node.obstacleVertice_Index == -1) return;
        ObstacleVertice obstacle1 = obstacles[node.obstacleVertice_Index];
        ObstacleVertice obstacle2 = obstacles[obstacle1.next_];
    ...
    private static void InsertObstacleNeighbor(ObstacleVertice obstacle, NativeList<ObstacleVertice> obstacles, NativeList<ObstacleVerticeNeighbor> obstacleNeighbors_, Agent agent, FixedInt rangeSq)
    {
