...
public partial class RVOSystem  
{
    ...
    private static void AddObstacleLine( Agent agent, NativeList<Line> orcaLines_, NativeList<ObstacleNeighbor> obstacleNeighbors_, NativeList<Obstacle> obstacles)
    {
            ...
            for (int i = 0; i < obstacleNeighbors_.Length; ++i)
            {
                ...
                FixedInt t = obstacle1 == obstacle2 ? (FixedInt)0.5f : ((velocity_ - leftCutOff) * cutOffVector) / RVOMath.absSq(cutOffVector);
        ...
        FixedInt invTimeHorizon = 1 / agent.timeHorizon_;
         /* Create agent ORCA lines. */
            for (int i = 0; i < agentNeighbors_.Length; ++i)
            {
                ...
                line.point = agent.velocity_ + (FixedInt)0.5f * u;
                orcaLines_.Add(line);
            }
    ...
    private static void ComputeNewVelocity(ref Agent agent, NativeList<Line> orcaLines_, NativeList<AgentNeighbor> agentNeighbors_, NativeList<ObstacleNeighbor> obstacleNeighbors_){
        ...
        #region agent 
            ...
            //     linearProgram3(orcaLines_, numObstLines, lineFail, agent.maxSpeed_, ref agent.newVelocity_);
            // }
            // Debug.Log(string.Format("{0},   " , agent.newVelocity_));
                    ...
                    for (int j = numObstLines; j < i; ++j)
                    {
                        ...
                        if (RVOMath.fabs(determinant) <= (FixedInt)(long) 1<<5)
                        {
                            ...
                            else
                            {
                                ...
                                line.point = (FixedInt)0.5f * (lines[i].point + lines[j].point);
                            }
                        }
                        else
                        {
