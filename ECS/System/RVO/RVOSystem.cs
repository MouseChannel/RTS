...
#region  compute Job
        ...
        Entities.ForEach((Entity entity, ref Agent agent) =>{
            ...
            jobHandleList.Add(computeNeighborsAndVelocity.Schedule());
        ...
        if (end - begin > MAX_LEAF_SIZE)
            {
                ...
                FixedInt splitValue = (FixedInt)0.5f * (isVertical ? agentTree_[node].maxX_ + agentTree_[node].minX_ : agentTree_[node].maxY_ + agentTree_[node].minY_);
        ...
        int length = current.Length;
            ...
            for (int i = 0; i < current.Length; ++i)
            {
                ...
                for (int j = 0; j < current.Length; ++j)
                {
                    ...
                    if (j1LeftOfI >= -(FixedInt)0.0001 && j2LeftOfI >= -FixedCalculate.superSmallValue)
                    {
                        ++leftSize;
                    }
                    else if (j1LeftOfI <= (FixedInt)0.0001 && j2LeftOfI <= FixedCalculate.superSmallValue)
                    {
                        ++rightSize;
                    }
                    else
